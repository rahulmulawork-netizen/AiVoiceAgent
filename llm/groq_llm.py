...
async def run_llm(
    ...
    If `logger` (TranscriptLogger) is provided, each completed assistant
    response is appended to the transcript file — one line per consolidated
    turn, never per-token or per-sentence.
    """
...
async def _chat_turn(
    ...
    await _chat_turn(history, text_queue, logger)
