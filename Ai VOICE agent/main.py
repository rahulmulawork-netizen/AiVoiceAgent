...
async def twilio_handler(twilio_ws) -> None:
    ...
    stt_task = asyncio.create_task(run_stt(audio_queue, transcript_queue, barge_in_event))
    llm_task = asyncio.create_task(run_llm(transcript_queue, text_queue))
    tts_task = asyncio.create_task(run_tts(text_queue, twilio_ws, streamsid, barge_in_event))
